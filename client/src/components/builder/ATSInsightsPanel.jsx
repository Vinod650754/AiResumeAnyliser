export const ATSInsightsPanel = ({ analysis }) => {
  if (!analysis) {
    return (
    ...
    <div className="liquid-glass space-y-5 rounded-[30px] p-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          {analysis.skillMatchScore ? <p className="mt-1 text-sm text-cyan-200/70">{analysis.skillMatchScore}% target skill match</p> : null}
          ...
          {analysis.targetRole ? <p className="mt-1 text-xs uppercase tracking-[0.22em] text-white/42">Target role: {analysis.targetRole}</p> : null}
        </div>
      ...
      <div>
        <p className="mb-3 text-sm font-semibold text-white">Matched skills</p>
        <div className="flex flex-wrap gap-2">
          {analysis.matchedSkills?.length ? (
            ...
            <span className="text-sm text-white/55">No target skills matched yet.</span>
          )}
        </div>
      </div>
      ...
      {analysis.rewrittenSummary ? (
        <div>
          <p className="mb-3 text-sm font-semibold text-white">AI rewritten summary</p>
          <p className="text-sm leading-7 text-white/72">{analysis.rewrittenSummary}</p>
        </div>
      ) : null}
    </div>
  );
